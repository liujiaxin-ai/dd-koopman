...
| path | contents |
|---|---|
| `run_csv/` | one CSV per evaluation run — one row per test setting, per-horizon NMSE values in the `nmse_mean` array column. These 159 files plus `reference/` are the only inputs of the analysis scripts |
...
| `logs/` | training-log excerpts backing prose-only facts: `s2_mamba64conv_excerpt.log` documents the Table 1 footnote "last improvement at epoch 93 of 100" for the MambaCSP-plateau baseline (source-file SHA256 in the header; full log stays on the training host) |
